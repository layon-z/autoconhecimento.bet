...
// Chave usada nas chamadas de Auth: usa a anon se existir, senão a service key
// (ambas autenticam a requisição ao projeto; o endpoint público /signup continua
//  respeitando o "Confirm email" do projeto, então a verificação por e-mail é mantida).
function authKey() {
  return ENV.SUPABASE_ANON_KEY || ENV.SUPABASE_KEY;
}

export function hasAuth() {
  return Boolean(ENV.SUPABASE_URL && authKey());
}

// ---------- Supabase Auth (GoTrue): cadastro, login, verificação por e-mail ----------
export async function gotrueSignup(email, password, username, redirectTo) {
  const qs = redirectTo ? `?redirect_to=${encodeURIComponent(redirectTo)}` : '';
  const r = await fetch(`${ENV.SUPABASE_URL}/auth/v1/signup${qs}`, {
    method: 'POST',
...
export async function gotruePasswordLogin(email, password) {
...
// Remove o usuário do Supabase Auth (usa a service key). Best-effort.
...
// Cache em memória (por instância) p/ reduzir chamadas ao football-data
let _matchesCache = { at: 0, data: null };

export async function getMatches() {
  const now = Date.now();
  if (_matchesCache.data && now - _matchesCache.at < 30000) return _matchesCache.data;
  ...
  if (ENV.FOOTBALL_DATA_KEY) {
    try {
      ...
      if (r.ok) {
        ...
        const shaped = (data.matches || []).map((m) =>
          shape({
            id: m.id,
            utcDate: m.utcDate,
            status: m.status,
        ...
        _matchesCache = { at: now, data: shaped };
        return shaped;
      }
    } catch (_) { /* falha temporária */ }
    // Chave configurada: NUNCA cair pros exemplos. Serve último dado bom ou vazio.
    return _matchesCache.data || [];
  }

  // Sem chave (modo demonstração) → jogos de exemplo
  const sample = SAMPLE_FIXTURES.map(shape);
  _matchesCache = { at: now, data: sample };
  return sample;
}
...
let _standingsCache = { at: 0, data: null };

export async function getStandings() {
  if (!ENV.FOOTBALL_DATA_KEY) return [];
  const now = Date.now();
  if (_standingsCache.data && now - _standingsCache.at < 90000) return _standingsCache.data;
  try {
    ...
    if (r.ok) {
      ...
      _standingsCache = { at: now, data: mapped };
      return mapped;
    }
  } catch (_) { /* falha temporária */ }
  return _standingsCache.data || []; // serve último dado bom em vez de vazio
}
